...
public class ASTGreaterOrEqual implements ASTNode {
    ...
    public ASTGreaterOrEqual(ASTNode l, ASTNode r, Context ctx) {
        left = l;
        right = r;
        this.ctx = ctx;
    }
    ...
    public Expr getAssertion(boolean assertion_context) {
    	if(right instanceof ASTCall)
    		return ctx.mkGe((ArithExpr) left.getAssertion(assertion_context), ctx.mkInt(0));
    	else if(left instanceof ASTCall)
    		return ctx.mkGe(ctx.mkInt(0), (ArithExpr) right.getAssertion(assertion_context));
	
    	return ctx.mkGe((ArithExpr) left.getAssertion(assertion_context), (ArithExpr) right.getAssertion(assertion_context));
